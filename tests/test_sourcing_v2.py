...
class TestAmazonSearchPlaceholder:
    ...
    def test_check_url_amazon_search_no_http(self):
        ...
        with patch("sourcing_health.cache_get", return_value=None), \
...
class TestOfflineAndQuotaBoundaries:
    def test_import_does_not_create_host_cache(self, tmp_path):
        scripts = Path(__file__).resolve().parent.parent / "scripts"
        home = tmp_path / "home"
        home.mkdir()

        result = subprocess.run(
            [sys.executable, "-c", "import sourcing_routes"],
            cwd=tmp_path,
        ...
        assert not (home / ".cache" / "electronics-stack").exists()
    ...
    def test_provider_imports_do_not_mutate_host(
        self,
        module_name,
        tmp_path,
    ):
        scripts = Path(__file__).resolve().parent.parent / "scripts"
        home = tmp_path / module_name
        home.mkdir()
        ...
        assert result.returncode == 0
        assert not (home / ".cache" / "electronics-stack").exists()
        assert not (home / ".config" / "electronics-stack").exists()

    def test_provider_routes_are_opt_in(self):
        url = "https://www.digikey.com/en/products/detail/mfr/PART/123"
        mock_module = MagicMock()
        mock_module.DigikeyClient.from_env.side_effect = AssertionError(
            "provider access was not authorized"
        )
        ...
        assert result["via"] == "head"
        mock_module.DigikeyClient.from_env.assert_not_called()

    def test_browserbase_escalation_is_opt_in(self):
        url = "https://example.invalid/anti-scrape"
        blocked = {
            "status": "status_503",
            "code": 503,
            "via": "head",
        ...
        with patch("sourcing_health.cache_get", return_value=None), \
             ...
             patch("sourcing_health.cache_set"), \
             patch("sourcing_health.http_check", return_value=blocked), \
             patch(
                 "sourcing_health.browserbase_fetch",
                 side_effect=AssertionError("Browserbase was not authorized"),
             ) as browserbase:
            result = sh.check_url(url)
    ...
    def test_browserbase_escalation_runs_when_enabled(self):
        url = "https://example.invalid/anti-scrape"
        blocked = {
            "status": "status_503",
            "code": 503,
        ...
        with patch("sourcing_health.cache_get", return_value=None), \
             ...
             patch("sourcing_health.cache_set"), \
             patch("sourcing_health.http_check", return_value=blocked), \
             patch("sourcing_health.browserbase_fetch", return_value=recovered):
            result = sh.check_url(url, allow_browserbase=True)

        assert result["status"] == "ok"
        assert result["via"] == "browserbase"

    def test_browserbase_unavailable_remains_non_actionable(self):
        url = "https://example.invalid/anti-scrape"
        blocked = {
            "status": "status_503",
            "code": 503,
        ...
        unavailable = {
            ...
            "escalated": True,
            "actionable": False,
            "note": "Browserbase authentication failed; URL remains unchecked",
        }
        ...
        with patch("sourcing_health.cache_get", return_value=None), \
             ...
             patch("sourcing_health.cache_set"), \
             patch("sourcing_health.http_check", return_value=blocked), \
             patch("sourcing_health.browserbase_fetch", return_value=unavailable):
            result = sh.check_url(url, allow_browserbase=True)
...
class TestLcscRouting:
    def test_lcsc_url_routes_to_db(self):
        ...
        mock_module = MagicMock()
        mock_module.LcscClient.from_env.return_value = mock_client
        ...
        assert result is not None
        assert result["status"] == "ok"
        assert result["via"] == "lcsc_db"
        mock_module.LcscClient.from_env.assert_called_once_with()
...
class TestBrowserbaseEscalation:
    ...
    def test_browserbase_fetch_success(self):
        ...
        with patch("sourcing_routes.subprocess.run", return_value=fake_proc), \
             patch.dict(os.environ, {"BROWSERBASE_CLI": "/fake/bb"}):
    ...
    def test_browserbase_failure_does_not_expose_stderr(self):
        url = "https://example.com/dead"
        fake_proc = MagicMock()
        fake_proc.returncode = 1
        fake_proc.stdout = ""
        fake_proc.stderr = "Authorization: Bearer FAKE-SHOULD-NOT-LEAK"

        with patch("sourcing_routes.subprocess.run", return_value=fake_proc), \
             patch.dict(os.environ, {"BROWSERBASE_CLI": "/fake/bb"}):
            result = routes.browserbase_fetch(url)

        assert "FAKE-SHOULD-NOT-LEAK" not in result.get("note", "")
