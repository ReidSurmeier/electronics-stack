...
The default route is local or ordinary HTTP only. Provider APIs and
Browserbase are separate, explicit opt-ins:
  1. Search-placeholder URLs (Amazon /s?k=, AliExpress wholesale) → marked OK, not fetched.
  2. Digikey product URLs → Digikey API v4 only with --with-api.
  3. Mouser product URLs  → Mouser API v2 only with --with-api.
  4. LCSC product URLs    → local jlcparts SQLite (if DB cached).
  5. Everything else      → HTTP HEAD with realistic UA rotation.
  6. 403/502/503          → Browserbase only with --with-browserbase
                            (capped at 30/BOM, cached 7d).

Usage:
    sourcing_health.py <BOM.xlsx> [--with-api] [--with-browserbase]
...
def check_url(
    ...
    *,
    ...
    Args:
        ...
        allow_provider_network: Permit Digikey/Mouser API routing.
        allow_browserbase: Permit a Browserbase fetch after anti-scrape blocks.
    ...
    routed = classify_url(url, allow_provider_network=allow_provider_network)
    ...
    if result.pop("_needs_escalation", False):
        if not allow_browserbase:
            result.update({
                ...
                "escalated": False,
                "note": "Browserbase disabled; URL remains unchecked",
            })
        elif escalation_budget[0] > 0:
...
def audit(
    ...
    Args:
        ...
        with_api: Permit provider URL routing and Digikey lifecycle checks.
        with_browserbase: Permit capped Browserbase anti-scrape escalation.
    ...
    for row in rows:
        ...
        for col in url_columns:
            ...
            result = check_url(
                ...
                allow_provider_network=with_api,
                allow_browserbase=with_browserbase,
            )
            result["item"] = item
            result["column"] = col
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "usage: sourcing_health.py <BOM.xlsx> "
            "[--with-api] [--with-browserbase]"
        )
        sys.exit(1)
    with_api = "--with-api" in sys.argv
    with_browserbase = "--with-browserbase" in sys.argv
    out = audit(
        sys.argv[1],
        with_api=with_api,
